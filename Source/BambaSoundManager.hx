...
class BambaSoundManager extends DisplayObject {
	...
	public function loadSoundsStart(param1:Xml):Void {
		...
		soundNames = [];
		soundsFiles = [];
		var gameSounds = param1.elementsNamed("gameSounds");
		var dugeonMusics = param1.elementsNamed("dungeonMusic");
	
		
		for (_loc3_ in gameSounds) {
			_loc2_ = _loc3_.get("name");
		...
		for (_loc4_ in dugeonMusics) {
			_loc2_ = _loc4_.get("name");
